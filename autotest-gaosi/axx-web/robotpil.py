...
#获取短信信息  
def getMysqlItem(strSql,strZd,strHost,strUser,strPwd,strDBname):
    """
    获取SQL查询结果中的第一行字段Item对应的内容
    Parameters:
    | getMysqlItem | SQL  |  Item   |  mysql.aixuexi.com   |  root   |  root123   |  managesystemv1.0   |
     """
    ...
    mysqlDB = mysqlDBUtil.dbUtil(strHost,strUser,strPwd)
...
#复制并替换文件
def replaceFiles(self,strNewFiles1,strOldFiles2):
    fp1 = open(strNewFiles1, "r")
    fp2 = open(strOldFiles2, "w")
    fp3 = fp1.readlines()  # 先读出来
    fp2.writelines(fp3)  # 替换 并写入
    fp1.close()
    fp2.close()
...
def scroll_down(self,locator):
        ...
        self._current_browser().execute_script("arguments[0].scrollIntoView();", element)
